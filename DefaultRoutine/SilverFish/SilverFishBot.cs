...
namespace HREngine.Bots
{
    public class SilverFishBot
    {
        ...
        public bool updateEverything(Behavior botbase, int numcal, out bool sleepRetry)
        {
            ...
            p.enemyCardsOut = new Dictionary<CardIdEnum, int>(Probabilitymaker.Instance.enemyCardsOut);
        ...
        private void getDecks()
        {
            ...
            for (int i = 0; i < allcardscount; i++)
            {
                ...
                if (CardDB.Instance.cardIdstringToEnum(entity.Id) == CardIdEnum.UNG_116t) ownMinionsCost0 = true;

                if (entity.GetZone() == Triton.Game.Mapping.TAG_ZONE.GRAVEYARD)
                {
                    CardIdEnum cide = CardDB.Instance.cardIdstringToEnum(entity.Id);
                    ...
                    if (cide == CardIdEnum.UNG_067t1) ownCrystalCore = 5;
                }
                ...
                if (entityId == "")
                {
                    if (a != null)
                    {
                        switch (a.actionType)
                        {
                            case actionEnum.playcard:
                                ...
                                {
                                    case CardIdEnum.LOE_104: goto case CardIdEnum.BRM_007; 
                                    case CardIdEnum.BRM_007: 
                                        if (a.target != null) entityId = a.target.handcard.card.cardIDenum.ToString();
                                        break;
                                    case CardIdEnum.LOE_002: entityId = "LOE_002t"; break; 
                                    case CardIdEnum.LOE_079: entityId = "LOE_019t"; break; 
                                    case CardIdEnum.LOE_019t: entityId = "LOE_019t2"; break;
                                    case CardIdEnum.LOE_110: entityId = "LOE_110t"; break; 
                                }
                                break;
                        }
                    }
                    if (entityId == "")
                    {
                        ...
                        foreach (var tmp in eg)
                        {
                            ...
                            switch (tmp.Key)
                            {
                                case CardIdEnum.AT_035: entityId = "AT_035t"; break; 
                                case CardIdEnum.GVG_031: entityId = "aiextra1"; break; 
                                case CardIdEnum.LOE_111: entityId = "LOE_111"; break; 
                            }
                        }
                        if (entityId == "" && lastpf != null)
                        {
                            ...
                            {
                                if (m.handcard.card.cardIDenum == CardIdEnum.GVG_056) num++; 
                            }
                            if (num > 0)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if (m.handcard.card.cardIDenum == CardIdEnum.GVG_035) num++; 
                                }
                                if (num > 0)
                                {
                                    foreach (Minion m in this.ownMinions)
